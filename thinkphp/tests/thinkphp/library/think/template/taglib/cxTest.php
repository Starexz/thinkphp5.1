...
EOF;
        ...
        $content = <<<EOF
{foreach name=":explode(',', '1,2,3,4,5')" id="val" key="key" Home="Home" mod="2" offset="1" length="3"}
{\$val}
{/foreach}
EOF;
...
EOF;
    ...
    public function testUrl()
    {
        $template = new template();
        $content  = <<<EOF
{url link="IndexController.class/Home"  /}
EOF;
        $template->display($content);
        $this->expectOutputString(\think\Url::build('IndexController.class/Home'));
    }
